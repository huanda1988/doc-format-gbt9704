...
`doc-format-gbt9704` 是一个用于 **GB/T 9704-2012《党政机关公文格式》** 的文档排版 Skill/Agent 规则包。它最初按 Codex Skill 结构组织，同时也适用于 OpenClaw、WorkBuddy、Claude Code 等能够读取 Markdown 指令、执行脚本并处理本地文件的 AI Agent。

它可以帮助 Agent 将 Word 文档、旧版 `.doc` 文件、纯文本草稿、工作总结、通知、报告、函、命令、纪要等材料，整理为符合党政机关公文格式要求的规范版式。

这个项目的目标不是简单“美化文档”，而是把公文排版中的纸张、版心、字体、字号、层级、页码、署名、日期、附件、版记等规则转化为 AI Agent 可以理解和执行的工作流。

## 主要用途

使用这个 Skill/规则包，可以让 Codex、OpenClaw、WorkBuddy、Claude Code 等 Agent 执行以下任务：
...
当你希望 AI Agent 完成类似下面的任务时，可以使用本 Skill：
...
Codex Skill 的入口文件，也可以作为其他 Agent 的主提示词说明文件。它包含 Skill 名称、触发描述、工作流、核心排版原则和资源说明。Codex 识别到相关任务时，会优先读取这个文件；其他 Agent 可直接读取该文件作为任务规范。

### `agents/openai.yaml`

Skill 在 Codex/OpenAI 相关界面中的展示元数据，包括显示名称、简短说明和默认提示词。其他 Agent 通常可以忽略该文件。
...
对于 OpenClaw、WorkBuddy、Claude Code 等 Agent，可以将本仓库克隆到任意工作目录，然后在提示词中要求 Agent 读取 `SKILL.md` 和 `references/format_rules.md`：

```bash
git clone https://github.com/huanda1988/doc-format-gbt9704.git
```

推荐提示词：

```text
请读取 doc-format-gbt9704/SKILL.md，并在需要详细规则时读取 doc-format-gbt9704/references/format_rules.md，然后把这份文档按 GB/T 9704-2012《党政机关公文格式》排版。
```

如果 Agent 支持执行本地脚本，也可以让它直接使用：

```bash
python doc-format-gbt9704/scripts/format_docx_gbt9704.py input.docx --out output.docx
```

## 在 Agent 中使用

在 Codex 中可以显式提到 Skill 名称：

```text
$doc-format-gbt9704 请把这份 Word 文档按党政机关公文格式排版。
```

在 OpenClaw、WorkBuddy、Claude Code 等 Agent 中，可以明确指定规则文件路径：

```text
请按照 doc-format-gbt9704/SKILL.md 和 references/format_rules.md 的要求，排版当前 Word 文档。
```

如果任务涉及 Word 文件，建议使用具备 DOCX 读写和渲染能力的工具链。理想流程是：
